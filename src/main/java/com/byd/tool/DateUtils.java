...
public class DateUtils {
     ...
     * @return
     */
    public static Integer betweenDateMM(Date startDate, Date endDate) {
        String startYear = dateToStr(startDate, "yyyy");
        String endYear = dateToStr(endDate, "yyyy");
        Integer yearMonth = (Integer.valueOf(endYear) - Integer.valueOf(startYear)) * 12;
        
        String startMonth = dateToStr(startDate, "MM");
        String endMonth = dateToStr(endDate, "MM");
        Integer month = Integer.valueOf(endMonth) - Integer.valueOf(startMonth);
     ...
     * @return
     */
    public static Double monthAge(Date birthday, Date nowDate) {
        Integer betweenMM = betweenDateMM(birthday, nowDate);
        String birthdayDay = dateToStr(birthday, "d");
        String nowDay = dateToStr(nowDate, "d");
        
        Date monthFirstDate = monthFirstDay(nowDate, 0);
        Date monthLastDate = monthLastDay(nowDate, 0);
        Integer monthDayCount = betweenDateDD(monthFirstDate, monthLastDate);
        
        Double monthChange = (Double.valueOf(nowDay) - Double.valueOf(birthdayDay) + 1) / Double.valueOf(monthDayCount);
        return Double.valueOf(betweenMM) + monthChange;
    }
     ...
     * @return
     */
    public static Double yearAge(Date birthday, Date nowDate) {
        return monthAge(birthday, nowDate) / 12;
    }
     ...
     * @return
     */
    public static Date dateGapDays(Date date, double day) {
        long time = date.getTime();
        day = day * 24 * 60 * 60 * 1000;
        time = time + Math.round(day);
    ...
    public static String getWeekEndDay() {
        ...
        SimpleDateFormat endSdf = new SimpleDateFormat("yyyy-MM-dd  23:00:00");
     ...
     * 获得上周最后一天:yyyy-MM-dd  HH:mm:ss
     */
    public static Date getLastWeekEndDayOfDate() {
        Calendar c = Calendar.getInstance();
        c.set(Calendar.DAY_OF_WEEK, 2);
        c.add(Calendar.DATE, -1);
        SimpleDateFormat endSdf = new SimpleDateFormat("yyyy-MM-dd  23:00:00");
        return strToDate(endSdf.format(c.getTime()));
    }
     ...
     * @return
     */
    public static Date toDate(Object timestamp) {
        if (timestamp == null) {
            return null;
        }
        
        if (timestamp instanceof Timestamp) {
            return new Date(((Timestamp) timestamp).getTime());
        } else if (timestamp instanceof Date) {
            return (Date) timestamp;
        } else {
            return strToDate(timestamp.toString());
        }
    }
    ...
    public static void main(String[] args) throws Exception {
        ...
        //        params.put("id","12") ;
        //        DJson result = new DJson(dHttp.POST(params,"http://localhost:8088/bigElephant/teacher/ass/v3/fieldCodeList") );
